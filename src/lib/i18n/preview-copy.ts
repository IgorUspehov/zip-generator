...
const DELIVERY_OPTIONS_EN: Record<DeliveryOptionKey, DeliveryOptionCopy> = {
  ...
  netlify: { label: "Netlify", description: "Netlify deployment" },
...
const DELIVERY_OPTIONS_RU: Record<DeliveryOptionKey, DeliveryOptionCopy> = {
  zip: { label: "ZIP", description: "final_package.zip" },
  netlify: { label: "Netlify", description: "Деплой на Netlify" },
...
const COPY: Record<Locale, PreviewCopy> = {
  ru: {
    ...
    domainGuideSubtitle: "Следуйте шагам, чтобы привязать домен к вашему MVP на Netlify",
    domainSteps: [
      "Купите домен у регистратора",
      "Откройте панель DNS вашего домена",
      "Добавьте DNS-записи Netlify (CNAME www + A @)",
      "Дождитесь проверки DNS и активации SSL",
    ],
    domainNetlifyHint: "Ваш Netlify-сайт:",
  ...
  de: {
    ...
    domainGuideSubtitle: "Folgen Sie diesen Schritten, um Ihre Domain mit Netlify zu verbinden",
    domainSteps: [
      "Domain beim Registrar kaufen",
      "DNS-Panel Ihrer Domain öffnen",
      "Netlify DNS-Einträge hinzufügen (CNAME www + A @)",
      "Auf DNS-Prüfung und SSL-Aktivierung warten",
    ],
    domainNetlifyHint: "Ihre Netlify-Site:",
  ...
  en: {
    ...
    domainGuideSubtitle: "Follow these steps to connect your domain to your MVP on Netlify",
    domainSteps: [
      "Buy a domain from a registrar",
      "Open your domain DNS panel",
      "Add Netlify DNS records (CNAME www + A @)",
      "Wait for DNS verification and SSL activation",
    ],
    domainNetlifyHint: "Your Netlify site:",
